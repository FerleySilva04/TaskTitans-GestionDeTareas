...
const DeleteTaskPopup: React.FC<DeleteTaskPopupProps> = ({ open, task, setOpen }) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      toast.error("Ha ocurrido un error al eliminar la tarea. Por favor, intenta nuevamente.");
    } finally {
      setOpen(false); // Cerrar el popup independientemente de si la eliminación fue exitosa o no
    }
  };
  ...
  return (
    ...
    >
      <DialogTitle id="delete-task-dialog-title">Eliminar Tarea</DialogTitle>
      <DialogContent>
        <Typography id="delete-task-dialog-description">
